...
def convert_marker_file(input_file, output_file):
    ...
    for line in lines:
        ...
        match = re.match(r'(\d\d)_(\w+)\t(\d\d:\d\d:\d\d\.\d{3})(\t(\d\d:\d\d:\d\d\.\d{3}))?', line)
        if match:
            name = match.group(2)
            start = match.group(3)
            ...
            if end:
                ...
                start = f'{int(h1):02d}:{int(m1):02d}:{s1:06.3f}'.rstrip('0').rstrip('.')
                end = f'{int(h2):02d}:{int(m2):02d}:{s2:06.3f}'.rstrip('0').rstrip('.')
